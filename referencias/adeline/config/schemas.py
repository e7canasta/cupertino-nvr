...
class EventSourcingSettings(BaseModel):
    """
    Event Sourcing configuration for stabilization debugging.

    Event sourcing logs all tracking events (appeared, updated, confirmed, missed, removed)
    for debugging and audit trail purposes.

    Philosophy: "Pragmatismo > Purismo"
    - Default OFF: No overhead in production
    - Enable for debugging: Track event history when troubleshooting
    - Degradation graceful: Same code works with/without event logging
    """
    enabled: bool = Field(
        default=False,
        description="Enable event sourcing (default OFF for performance)"
    )
    max_events: int = Field(
        default=1000,
        ge=100,
        le=10000,
        description="Maximum events in circular log (prevents unbounded growth)"
    )
    export_on_shutdown: bool = Field(
        default=False,
        description="Export event log to JSON on pipeline shutdown"
    )
    mqtt_export_enabled: bool = Field(
        default=False,
        description="Enable MQTT command for live event export"
    )
...
class StabilizationSettings(BaseModel):
    ...
    event_sourcing: EventSourcingSettings = Field(
        default_factory=EventSourcingSettings,
        description="Event sourcing config for debugging (optional)"
    )
...
class AdelineConfig(BaseModel):
    ...
    def to_legacy_config(self) -> 'PipelineConfig':
        ...
        legacy.EVENT_SOURCING_ENABLED = self.detection_stabilization.event_sourcing.enabled
        legacy.EVENT_SOURCING_MAX_EVENTS = self.detection_stabilization.event_sourcing.max_events
        legacy.EVENT_SOURCING_EXPORT_ON_SHUTDOWN = self.detection_stabilization.event_sourcing.export_on_shutdown
        legacy.EVENT_SOURCING_MQTT_EXPORT_ENABLED = self.detection_stabilization.event_sourcing.mqtt_export_enabled
