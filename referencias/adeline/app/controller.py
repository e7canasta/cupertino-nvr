...
class InferencePipelineController:
    ...
    def _setup_control_callbacks(self):
        ...
        if self.stabilizer is not None:
            ...
            # Comando STABILIZATION_EVENTS solo si event sourcing enabled Y mqtt export enabled
            if (hasattr(self.stabilizer, 'event_sourcing_enabled') and
                self.stabilizer.event_sourcing_enabled and
                hasattr(self.stabilizer, 'events') and
                self.stabilizer.events is not None):
                # Check si mqtt export está enabled (via config)
                if self.config.EVENT_SOURCING_MQTT_EXPORT_ENABLED:
                    registry.register('stabilization_events', self._handle_stabilization_events, "Export event log (JSON)")
                    logger.info("✅ stabilization_events command registered (event sourcing + MQTT export enabled)")
    ...
    def _handle_stabilization_events(self):
        """
        Callback para comando STABILIZATION_EVENTS - exporta event log vía MQTT.

        Philosophy: "Pragmatismo > Purismo"
        - Solo disponible si event_sourcing.enabled=true Y mqtt_export_enabled=true
        - Registro condicional (no aparece si disabled)
        - Export como JSON compacto para MQTT payload
        """
        logger.info("🎥 Comando STABILIZATION_EVENTS recibido")

        # Safety checks (redundantes, pero defensive programming)
        if not hasattr(self, 'stabilizer') or self.stabilizer is None:
            logger.warning("⚠️ Stabilizer no disponible")
            return

        if not hasattr(self.stabilizer, 'events') or self.stabilizer.events is None:
            logger.warning("⚠️ Event sourcing no habilitado")
            logger.info("💡 Para habilitar, configurar detection_stabilization.event_sourcing.enabled=true en config.yaml")
            return

        try:
            # Obtener events del stabilizer
            events = self.stabilizer.get_events()

            if events is None or len(events) == 0:
                logger.info("ℹ️ Event log vacío (no events registrados aún)")
                # Publicar respuesta vacía
                self.control_plane.publish_status(json.dumps({
                    'command': 'stabilization_events',
                    'event_count': 0,
                    'events': []
                }))
                return

            # Convert events to dict format (JSON-serializable)
            events_data = [event.to_dict() for event in events]

            # Log summary local
            logger.info(f"📊 Event Log Summary:")
            ...
            payload = {
                ...
                'events': events_data,
                'summary': dict(event_types),  # Counter → dict para JSON
            }

            # Publicar via control plane (compact JSON)
            payload_json = json.dumps(payload, separators=(',', ':'))  # Compact JSON
            payload_size_kb = len(payload_json) / 1024

            logger.info(f"📤 Publishing event log via MQTT ({payload_size_kb:.1f} KB)")

            self.control_plane.publish_status(payload_json)

            logger.info("✅ Event log exported successfully")

        except Exception as e:
            logger.error(
                "Failed to export stabilization events",
                extra={
                    "component": "pipeline_controller",
                    "event": "stabilization_events_error",
                    "error": str(e),
                    "error_type": type(e).__name__
                },
                exc_info=True
