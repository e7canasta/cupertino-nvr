...
class StabilizationConfig:
    ...
    # Event Sourcing params (OPCIONAL - para debugging)
    event_sourcing_enabled: bool = False          # Default OFF (no overhead)
    event_sourcing_max_events: int = 1000         # Circular log size
    event_sourcing_export_on_shutdown: bool = False   # Export JSON al terminar
    event_sourcing_mqtt_export_enabled: bool = False  # MQTT command para export
...
class TemporalHysteresisStabilizer(BaseDetectionStabilizer):
    ...
    def __init__(
        ...
        event_sourcing_max_events: int = 1000,
        event_sourcing_export_on_shutdown: bool = False,
    ):
        """
        Args:
            ...
            event_sourcing_enabled: Habilitar event logging (default: False)
            event_sourcing_max_events: Tamaño del event log circular (default: 1000)
            event_sourcing_export_on_shutdown: Export JSON al terminar (default: False)
        """
        ...
        # Event Sourcing (OPCIONAL - default OFF)
        self.event_sourcing_enabled = event_sourcing_enabled
        self.event_sourcing_export_on_shutdown = event_sourcing_export_on_shutdown
        if event_sourcing_enabled:
            self.events: Optional[deque] = deque(maxlen=event_sourcing_max_events)
            logger.info(
                "🎥 Event Sourcing ENABLED",
                extra={
                    "component": "stabilization",
                    "event": "event_sourcing_enabled",
                    "max_events": event_sourcing_max_events,
                    "export_on_shutdown": event_sourcing_export_on_shutdown,
                }
            )
        else:
            self.events = None  # Zero cost si disabled
        ...
        # Frame counter (para event logging)
        ...
        logger.info(
            ...
            extra={
                ...
                "event_sourcing_enabled": event_sourcing_enabled,
    ...
    def _log_event(
        ...
        """
        Log track event si event sourcing está enabled.

        Philosophy: "Zero cost si disabled"
        - Single `if self.events is not None` check es O(1)
        - No overhead de creación de TrackEvent si disabled

        Args:
            track: DetectionTrack con estado actual
            event_type: Tipo de event (appeared, updated, confirmed, missed, removed)
            reason: Razón del event (opcional, para debugging)
        """
        if self.events is not None:
            # Generar track_id único (class + spatial hash simple)
            track_id = f"{track.class_name}_{int(track.x * 1000)}_{int(track.y * 1000)}"
            ...
            event = TrackEvent(
                ...
                confidence=track.confidence if event_type != EVENT_REMOVED else 0.0,
    ...
    def process(
        ...
        """
        # Increment frame counter (para event logging)
        self._frame_id += 1
        ...
        for det in detections:
            ...
            if class_name in tracks:
                ...
                if match_result is not None:
                    ...
                    if confidence >= threshold:
                        ...
                        # Log EVENT_UPDATED (si no estaba confirmado aún, es tracking)
                        self._log_event(track, EVENT_UPDATED, f"matched via {strategy_name}, score={match_score:.2f}")
                        ...
                        if not track.confirmed and track.consecutive_frames >= self.min_frames:
                            ...
                            self._log_event(
                                track,
                                EVENT_CONFIRMED,
                                f"min_frames_met ({self.min_frames} consecutive frames)"
                            )
                        ...
                        track.mark_missed()
                        # Log EVENT_MISSED (confidence too low)
                        self._log_event(track, EVENT_MISSED, f"confidence {confidence:.2f} < threshold {threshold:.2f}")
            ...
            if not matched:
                if confidence >= self.appear_conf:
                    ...
                    # Log EVENT_APPEARED
                    self._log_event(new_track, EVENT_APPEARED, f"new detection, confidence={confidence:.2f}")
        ...
        for class_name, track_list in tracks.items():
            for idx, track in enumerate(track_list):
                if (class_name, idx) not in matched_tracks:
                    track.mark_missed()
                    # Log EVENT_MISSED (no detection this frame)
                    self._log_event(track, EVENT_MISSED, f"no match in frame, gap={track.gap_frames}/{self.max_gap}")
        ...
        for class_name in list(tracks.keys()):
            ...
            # Log EVENT_REMOVED para tracks expirados (antes de filtrar)
            for track in track_list:
                if track.gap_frames > self.max_gap:
                    self._log_event(track, EVENT_REMOVED, f"max_gap exceeded ({track.gap_frames} > {self.max_gap})")
    ...
    def get_stats(self, source_id: int = 0) -> Dict[str, Any]:
        ...
        # Agregar event sourcing stats si enabled
        if self.events is not None:
            stats['event_sourcing'] = {
                'enabled': True,
                'total_events': len(self.events),
                'max_events': self.events.maxlen,
            }
        else:
            stats['event_sourcing'] = {'enabled': False}

        return stats

    def get_events(self) -> Optional[List[TrackEvent]]:
        """
        Get event log (si event sourcing está enabled).

        Returns:
            Lista de TrackEvent, o None si event sourcing disabled

        Example:
            >>> stabilizer = TemporalHysteresisStabilizer(event_sourcing_enabled=True)
            >>> # ... process frames ...
            >>> events = stabilizer.get_events()
            >>> len(events)
            150
        """
    ...
    def export_events_to_file(self, filepath: str) -> bool:
        """
        ...
        Returns:
            True si export exitoso, False si event sourcing disabled

        Example:
            >>> stabilizer.export_events_to_file('stabilization_events.json')
            True
        """
        events = self.get_events()
        if events is None:
            logger.warning(
                "Cannot export events: event sourcing is disabled",
                extra={"component": "stabilization", "event": "export_failed"}
            )
            return False

        export_events_to_json(events, filepath)
        logger.info(
            f"Events exported to {filepath}",
            extra={
                "component": "stabilization",
                "event": "events_exported",
                "filepath": filepath,
...
def create_stabilization_strategy(
    ...
    if mode == 'temporal':
        ...
        return TemporalHysteresisStabilizer(
            ...
            persist_conf=config.hysteresis_persist_conf,
            iou_threshold=config.iou_threshold,
            event_sourcing_enabled=config.event_sourcing_enabled,
            event_sourcing_max_events=config.event_sourcing_max_events,
            event_sourcing_export_on_shutdown=config.event_sourcing_export_on_shutdown,
        )
